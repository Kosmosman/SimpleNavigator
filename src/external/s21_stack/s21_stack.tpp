...
template <typename T> Stack<T>::Stack() : head_(nullptr), size_(0) {}
...
template <typename T> Stack<T> &Stack<T>::operator=(const Stack &s) {
  if (this == &s)
    return *this;
...
template <typename T> typename Stack<T>::reference Stack<T>::top() {
  return head_->value;
}

/* ------------------------- CAPACITY ---------------------------- */

template <typename T> bool Stack<T>::empty() { return size() == 0; }

template <typename T> typename Stack<T>::size_type Stack<T>::size() {
  return size_;
}
...
template <typename T> void Stack<T>::swap(Stack &other) {
