...
TEST(ShortestWay, DeijkstraAlgoTest) {
    Testcases tk;
    int counter{};
    for (auto &[test_name, testcase]: tk.tk3x3) {
        auto res{GraphAlgorithms::GetShortestPathBetweenVertices(tk.g3, testcase.input.first, testcase.input.second)};
        if (res != testcase.want) {
            std::cout << "Wrong answer in test \"" << test_name << "\", expected: "
                      << testcase.want << ", result: " << res << '\n';
            ++counter;
        }
    }

    for (auto &[test_name, testcase]: tk.tk10x10) {
        auto res{GraphAlgorithms::GetShortestPathBetweenVertices(tk.g10, testcase.input.first, testcase.input.second)};
        if (res != testcase.want) {
            std::cout << "Wrong answer in test \"" << test_name << "\", expected: "
                      << testcase.want << ", result: " << res << '\n';
            ++counter;
    ...
    if (!counter) std::cout << "All test passed!\n";
}
